import React from 'react';
import type { Meta, StoryObj } from '@storybook/react';
import CheckIcon from './CheckIcon';

const meta = {
  title: 'Icons/Actions/CheckIcon',
  component: CheckIcon,
  parameters: {
    layout: 'centered',
  },
  tags: ['autodocs'],
  argTypes: {
    size: {
      control: 'select',
      options: ['sm', 'md', 'lg'],
      description: 'Size of the icon',
    },
    className: {
      control: 'text',
      description: 'Additional CSS classes for styling',
    },
  },
} satisfies Meta<typeof CheckIcon>;

export default meta;
type Story = StoryObj<typeof meta>;

/**
 * Default Check Icon
 * Shows the check circle icon in medium size
 */
export const Default: Story = {
  args: {
    size: 'md',
  },
};

/**
 * Small Check Icon
 * Useful for inline usage or compact layouts
 */
export const Small: Story = {
  args: {
    size: 'sm',
  },
};

/**
 * Large Check Icon
 * Suitable for prominently featured checkmarks
 */
export const Large: Story = {
  args: {
    size: 'lg',
  },
};

/**
 * Check Icon with Green Color
 * Commonly used for success states
 */
export const GreenColor: Story = {
  args: {
    size: 'md',
    className: 'text-green-600',
  },
};

/**
 * Check Icon with Blue Color
 */
export const BlueColor: Story = {
  args: {
    size: 'md',
    className: 'text-blue-600',
  },
};

/**
 * Check Icon with Custom Color
 */
export const CustomColor: Story = {
  args: {
    size: 'md',
    className: 'text-emerald-500',
  },
};

/**
 * Multiple Sizes Comparison
 */
export const AllSizes: Story = {
  render: () => (
    <div className="flex gap-4">
      <CheckIcon size="sm" />
      <CheckIcon size="md" />
      <CheckIcon size="lg" />
    </div>
  ),
};
