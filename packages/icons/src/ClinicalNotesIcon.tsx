import React from "react";

export type IconSize = "sm" | "md" | "lg" | "xl";

interface ClinicalNotesIconProps {
  size?: IconSize;
  className?: string;
}

const sizeMap: Record<IconSize, string> = {
  sm: "w-4 h-4",
  md: "w-5 h-5",
  lg: "w-6 h-6",
  xl: "w-8 h-8",
};

/**
 * ClinicalNotesIcon component
 * Displays a clinical notes icon
 */
const ClinicalNotesIcon: React.FC<ClinicalNotesIconProps> = ({
  size = "md",
  className = "",
}) => {
  return (
    <svg
      className={`${sizeMap[size]} ${className}`}
      fill="currentColor"
      viewBox="0 -960 960 960"
      xmlns="http://www.w3.org/2000/svg"
      aria-label="Clinical Notes"
    >
      <path d="M595-365q-35-35-35-85t35-85q35-35 85-35t85 35q35 35 35 85t-35 85q-35 35-85 35t-85-35Zm127.5-42.5Q740-425 740-450t-17.5-42.5Q705-510 680-510t-42.5 17.5Q620-475 620-450t17.5 42.5Q655-390 680-390t42.5-17.5ZM440-50v-116q0-21 10-39.5t28-29.5q28-17 58-29.5t62-20.5l82 106 82-106q32 8 61.5 20.5T881-235q18 11 28.5 29.5T920-166v116H440Zm60-60h157l-82-106q-20 8-39 17.5T500-178v68Zm203 0h157v-68q-17-11-35.5-20.5T786-216l-83 106Zm-46 0Zm46 0Zm-523-10q-24.75 0-42.37-17.63Q120-155.25 120-180v-600q0-24.75 17.63-42.38Q155.25-840 180-840h600q24.75 0 42.38 17.62Q840-804.75 840-780v247q-11-20-26-37t-34-30v-180H180v600h200v60H180Zm100-500h341q14-5 28.84-7.5T680-630v-50H280v60Zm0 170h220q0-15 2.5-30.5T510-510H280v60Zm0 170h158q17-13 36-21.5t39-16.5v-22H280v60ZM180-180v-600 180-30 450Zm500-270Z" />
    </svg>
  );
};

export default ClinicalNotesIcon;
