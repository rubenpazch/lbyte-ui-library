...
describe('Button Component', () => {
  ...
  describe('Focus Styles', () => {
    ...
    it('renders with outline focus style', () => {
      const { container } = render(<Button focusStyleType="outline">Button</Button>);
      expect(container.querySelector('button')).toHaveClass('focus:ring-2');
    });

    it('renders with underline focus style', () => {
      const { container } = render(<Button focusStyleType="underline">Button</Button>);
  ...
  describe('Focus Trigger Types', () => {
    it('sets data-focus-trigger attribute', () => {
      const { container } = render(<Button focusTriggerType="parent">Button</Button>);
      expect(container.querySelector('button')).toHaveAttribute('data-focus-trigger', 'parent');
    });

    it('defaults to self focus trigger', () => {
      const { container } = render(<Button>Button</Button>);
      expect(container.querySelector('button')).toHaveAttribute('data-focus-trigger', 'self');
    });
  });
  ...
  describe('Combined Props', () => {
    ...
    it('renders small circled disabled button', () => {
      ...
      const button = container.querySelector('button');
      expect(button).toHaveClass('px-3', 'py-1.5', 'rounded-full', 'bg-gray-300', 'text-gray-500');
    });
    ...
    it('renders large inverted gradient-green button', () => {
      ...
      const button = container.querySelector('button');
      expect(button).toHaveClass('px-6', 'py-3', 'bg-gradient-to-r', 'from-emerald-400');
    });
  });
  ...
  describe('HTML Attributes', () => {
    it('forwards all button HTML attributes', () => {
      const { container } = render(
        <Button title="Hover Title" aria-label="Custom Label" data-testid="custom-button">
          Button
        </Button>
      );
      const button = container.querySelector('button');
  ...
  describe('Hover and Transition', () => {
    it('has transition classes for smooth interaction', () => {
      const { container } = render(<Button>Button</Button>);
      expect(container.querySelector('button')).toHaveClass('transition-all', 'duration-200');
    });
    ...
    it('applies hover state to filled button', () => {
      ...
      expect(container.querySelector('button')).toHaveClass('hover:bg-blue-600');
    });
  });
